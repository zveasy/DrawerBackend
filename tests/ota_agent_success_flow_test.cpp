...
TEST(OtaAgent, SuccessFlow) {
  path tmp = temp_directory_path()/"ota_success"; remove_all(tmp); create_directories(tmp);
  ...
  std::ofstream(state/"pub.pem")<<pub;

  path artifact = tmp/"artifact.bin"; std::string adata="hello"; std::ofstream(artifact)<<adata;
  std::string sha = hex_sha(adata);
  std::string payload = std::string("{\"channel\":\"stable\",\"version\":\"1.1.0\",\"artifact_url\":\"file://") + artifact.string() + "\",\"sha256\":\""+sha+"\"}";
  ...
  ota::Agent agent(c,*backend);
  auto res = agent.run_once();
  ASSERT_TRUE(res.ok);
  std::map<std::string,std::string> kv; persist::load_kv((state/"state.json").string(),kv);
  EXPECT_EQ(kv["pending_version"],"1.1.0");
  EXPECT_EQ(kv["boot_pending"],"1");

  httplib::Server svr; svr.Get("/healthz",[](const httplib::Request&,httplib::Response& r){ r.set_content("{\"ok\":true}","application/json"); });
  int port = svr.bind_to_any_port("127.0.0.1");
  std::thread th([&]{ svr.listen_after_bind(); });

  ota::Healthd hd(c,*backend);
  auto res2 = hd.run_once("http://127.0.0.1:"+std::to_string(port));
  svr.stop(); th.join();
  EXPECT_TRUE(res2.ok);

  kv.clear(); persist::load_kv((state/"state.json").string(),kv);
  EXPECT_EQ(kv["current_version"],"1.1.0");
